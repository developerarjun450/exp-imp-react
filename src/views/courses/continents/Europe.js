...
import EuropeController from '../../../controllers/Admin/Course/Continent/EuropeController'
const Cards = () => {
  ...
  const fetchData = async () => {
    ...
    try{
      ...
      response = await EuropeController.fetchCountryList(token);
  ...
  const handleDetailsClick = async(countryId) => {
    let response = [];
    try{
      response = await EuropeController.fetchInstituteDetailsByCountry(token,'Europe',countryId)
      navigate('/admin/courses/continents/educational-institutes', { state: { institueData: response.data, continent: 'Europe', countryId: countryId } });
        ...
        <CCard className="mb-4">
          ...
          <CCardBody>
            <p className="text-body-secondary small">
              Listed all countries under Europe.
            </p>
